...
def pearsonr(x, y):
    ...
    assert len(x) == len(
        y
    ), "Prediction and gold standard does not have the same length..."
...
def calculate_metrics(golds, predictions, task1, task2, task3, task4):
    ...
    if task2:
        ...
        (
            ...
            macroF,
            accuracy,
        ) = calculatePRF(gold_emo, prediction_emo)
    else:
        (
            microrecall,
            microprecision,
...
def score(input_dir, output_dir):
    ...
    task1, predictedList_EMP = read_file(
        submission_path=submission_path, nb_labels=nb_labels_EMP, nb_samp=nb_samp
    )

    submission_path = os.path.join(input_dir, "res", "predictions_EMO.tsv")
    task2, predictedList_EMO = read_file(
        submission_path=submission_path, nb_labels=nb_labels_EMO, nb_samp=nb_samp
    )

    submission_path = os.path.join(input_dir, "res", "predictions_PER.tsv")
    task3, predictedList_PER = read_file(
        submission_path=submission_path, nb_labels=nb_labels_PER, nb_samp=nb_samp
    )

    submission_path = os.path.join(input_dir, "res", "predictions_IRI.tsv")
    task4, predictedList_IRI = read_file(
        submission_path=submission_path, nb_labels=nb_labels_IRI, nb_samp=nb_samp
    )

    predictedList = [
        i + j + k + l
        for i, j, k, l in zip(
            predictedList_EMP, predictedList_EMO, predictedList_PER, predictedList_IRI
        )
    ]
    ...
    (
        ...
        avg_pearson_PER,
        avg_pearson_IRI,
    ) = calculate_metrics(goldList, predictedList, task1, task2, task3, task4)
